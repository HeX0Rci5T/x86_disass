...
__u8 get_signed_char(__u64 v, __u64 sz) {
	return (v > (__u64)N_BITS_MAX(sz)/2) ? '-' : '+';
}
	...
	foreach_operand(p, in) {
		...
		if (p->is_imm) {
			if (!!p->rip) {
				...
				prf("%c0x%llx", get_signed_char(*(__u64*)p->imm, p->sz), v);
				prf("]");
			} else {		// is IMM
			...
			if (p->sib.on) {
				...
				prf("%c0x%llx", get_signed_char(*(__u64*)p->disp, p->disp_sz), v);
				prf("]");
			}
		}
	}
